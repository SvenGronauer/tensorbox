...
def create_mnist_dataset(train_val_split=0.8, apply_preprocessing=True):
    ...
    if apply_preprocessing:
        ds_train = ds_train.map(convert_rgb_images_to_float).shuffle(256).batch(32)
        ds_val = ds_val.map(convert_rgb_images_to_float).batch(32)
